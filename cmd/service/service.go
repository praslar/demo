...
import (
	...
	repo "development-kit/pkg/repo"
	service "development-kit/pkg/service"
	utils "development-kit/pkg/utils"
	"flag"
	"fmt"
	"net"
	http2 "net/http"
...
var fs = flag.NewFlagSet("development-kit", flag.ExitOnError)
...
func Run() {
	...
	if *zipkinURL != "" {
		...
		endpoint, err := zipkingo.NewEndpoint("development-kit", "localhost:80")
		if err != nil {
			logger.Log("err", err)
			os.Exit(1)
		}
	...
	db := repo.NewBasicPostgresDatabase(d)
	svc := service.New(getServiceMiddleware(logger), db)
	eps := endpoint.New(svc, getEndpointMiddleware(logger))
	g, err := createService(eps)
	if err != nil {
		return
	}
	_ = model.AutoMigration()
	utils.LoadEnv()
	err = initMetricsEndpoint(g)
	if err != nil {
		return
	}
...
func initHttpHandler(endpoints endpoint.Endpoints, g *group.Group) error {
...
func initMetricsEndpoint(g *group.Group) error {
